...
class MultiScaleSeasonMixing(nn.Module):
    ...
    def __init__(self, configs):
        ...
        #     ]
        # )
        isometric_kernel = [17, 49, 81]
        conv_kernel = [12, 16, 18]
        # isometric convolution
        # self.isometric_conv = nn.ModuleList([nn.Conv1d(in_channels=configs.seq_len // (configs.down_sampling_window ** i), out_channels=configs.seq_len // (configs.down_sampling_window ** (i + 1)),
        #                                                kernel_size=l, padding=0, stride=1)
        #                                      for l,i in zip(isometric_kernel,range(len(isometric_kernel)))
        #                                      ])
        #
...
class PastDecomposableMixing(nn.Module):
    def __init__(self, configs):
        ...
        # )
        # self.out_cross_layer = nn.Sequential(
        #     nn.Conv1d(in_channels=configs.seq_len, out_channels=configs.d_ff,kernel_size=1),
        #     nn.ReLU(),
        #     nn.Conv1d(in_channels=configs.d_ff, out_channels=configs.seq_len,kernel_size=1),
        # )
