...
class Seasonal_Prediction(nn.Module):
    ...
    def forward(self, dec):
        dec  = dec.permute(0,2,1)  # 32* 128 * 12（7+5）
...
'''
    def forward(self, dec):
        dec  = dec.permute(0,2,1)  # 32* 128 * 12（7+5）
        multi = []
        for i in range(len(self.decomp_kernel)):
            season_out, trend_out = self.decomp_layer[i](dec)

            x_enc, x_mark_enc = self.__multi_scale_process_inputs(season_out,None)
            x_list = []
            x_mark_list = []
            if x_mark_enc is not None:
                for i, x, x_mark in zip(range(len(x_enc)), x_enc, x_mark_enc):
        ...
        result = torch.stack(multi, dim=-1).sum(-1)
        # result = self.merge_outer(multi)
        result = self.normalize_layers[0](result, 'denorm')
