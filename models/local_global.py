    ...
    def __init__(self, configs):
        ...
        self.down_sampling_layers = torch.nn.ModuleList(
            [
                nn.Sequential(
                    torch.nn.Conv1d(
                        in_channels=configs.seq_len // (configs.down_sampling_window ** i),
                        out_channels=configs.seq_len // (configs.down_sampling_window ** (i + 1)),
                        kernel_size=1
                    ),
                    nn.ReLU(),
                    torch.nn.Conv1d(
                        in_channels=configs.seq_len // (configs.down_sampling_window ** (i + 1)),
                        out_channels=configs.seq_len // (configs.down_sampling_window ** (i + 1)),
                        kernel_size=1,
                    ),
    ...
    def forward(self, x_list):
        ...
        for i, ori, out_season, out_trend, length in zip( range(len(x_list)),x_list, out_season_list, out_trend_list,
            ...
            if self.channel_independence:
                # out = ori + self.out_cross_layer_test(out.permute(0,2,1)).permute(0,2,1)
...
class Seasonal_Prediction(nn.Module):
    def __init__(self, configs,embedding_size=512, n_heads=8, dropout=0.05, d_layers=1, decomp_kernel=[32], c_out=1,
        ...
        if self.channel_independence == 1:
            ...
            # self.projection_layer = nn.AdaptiveAvgPool1d( 1)
