...
class PastDecomposableMixing(nn.Module):
    ...
    def forward(self, x_list):
        ...
        for x in x_list:
            ...
            # season, trend = self.decompsition(season)
            # season = x
...
class Seasonal_Prediction(nn.Module):
    def __init__(self, configs, embedding_size=512, n_heads=8, dropout=0.05, d_layers=1, decomp_kernel=[32], c_out=1,
        ...
        self.fourier_layer = FourierLow(seq_len=configs.seq_len ,individual=False,enc_in=configs.enc_in,cut_freq=50)
    ...
    def forward(self, dec,x_mark):
        ...
        # t = multi
        # merge
        dec_out = self.fourier_layer(dec_out)
